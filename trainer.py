...
class Trainer():
    ...
    def _train(self):
        ...
        for i, (inputs, labels) in enumerate(self.train_data_loader):              # Notice
            ...
            loss = self.loss_fn[0](outputs, labels) #cross entropy
            #loss = self.loss_fn(outputs, labels) #focal loss
            if self.metric is not None:
    ...
    def _save_best_model(self):
        ...
        torch.save(state, './checkpoint/' + self.model_type + 'epoch_%d' % self.cur_epoch + 'best_acc'+ '.ckpt')   # Notice